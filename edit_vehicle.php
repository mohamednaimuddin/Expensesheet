...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE vehicle SET brand=?, model=?, number_plate=?, model_year=?, vehicle_owner=?, date_purchase=?, insurance_exp=?, authorised_user=?, auth_start=?, auth_expire=?, insurance_policy=?, estimara_exp=?, fas_exp=? WHERE id=?");
    ...
    if ($stmt->execute()) {
        ...
        echo "<div class='alert alert-danger text-center'>Error updating: " . $conn->error . "</div>";
    }
}
?>
<!DOCTYPE html>
...
<title>Edit Vehicle | VisionAngles</title>
...
<div class="container my-4">
    <h2 class="mb-3">✏️ Edit Vehicle</h2>
    <form method="POST" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="number_plate" class="form-control" value="<?= htmlspecialchars($vehicle['number_plate']) ?>" required>
        </div>

        <div class="col-md-6">
            <label class="form-label">Model Year</label>
            <input type="number" name="model_year" class="form-control" value="<?= htmlspecialchars($vehicle['model_year']) ?>" required>
        </div>

        <div class="col-md-6">
            <label class="form-label">Vehicle Owner</label>
            <input type="text" name="vehicle_owner" class="form-control" value="<?= htmlspecialchars($vehicle['vehicle_owner']) ?>" required>
        </div>

        <div class="col-md-6">
            <label class="form-label">Date of Purchase</label>
            <input type="date" name="date_purchase" class="form-control" value="<?= htmlspecialchars($vehicle['date_purchase']) ?>" required>
        </div>

        <div class="col-md-6">
            <label class="form-label">Insurance Expiry</label>
            <input type="date" name="insurance_exp" class="form-control" value="<?= htmlspecialchars($vehicle['insurance_exp']) ?>" required>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="authorised_user" class="form-control" value="<?= htmlspecialchars($vehicle['authorised_user']) ?>">
        </div>

        <div class="col-md-6">
            <label class="form-label">Authorization Start</label>
            <input type="date" name="auth_start" class="form-control" value="<?= htmlspecialchars($vehicle['auth_start']) ?>">
        </div>

        <div class="col-md-6">
            <label class="form-label">Authorization Expire</label>
            <input type="date" name="auth_expire" class="form-control" value="<?= htmlspecialchars($vehicle['auth_expire']) ?>">
        </div>

        <div class="col-md-6">
            <label class="form-label">Estimara Expiry</label>
            <input type="date" name="estimara_exp" class="form-control" value="<?= htmlspecialchars($vehicle['estimara_exp']) ?>">
        </div>

        <div class="col-md-6">
            <label class="form-label">FAS Expiry (1 yr)</label>
            <input type="date" name="fas_exp" class="form-control" value="<?= htmlspecialchars($vehicle['fas_exp']) ?>">
        </div>
